  ...
  return (
    <>
          ...
          <Box component="form" onSubmit={handleSubmit(onSubmit)} sx={{ display: "flex", flexDirection: "column", gap: 2, pt: 1 }}>
            <Box sx={{ display: "flex", gap: 2 }}>
              <Box sx={{ flex: 1 }}>
                ...
                <TextField fullWidth size="small" {...register("firstName", { required: "Prénom requis" })} error={!!errors.firstName} helperText={errors.firstName?.message} />
              </Box>
              <Box sx={{ flex: 1 }}>
                <FormLabel>Nom *</FormLabel>
                <TextField fullWidth size="small" {...register("lastName", { required: "Nom requis" })} error={!!errors.lastName} helperText={errors.lastName?.message} />
              </Box>
            </Box>
            ...
            <Box>
              ...
              <FormGroup row>
                <FormControlLabel control={<Checkbox checked={selectedRoles.includes("EMPLOYEE")} onChange={() => toggleRole("EMPLOYEE")} />} label="Employé" />
                <FormControlLabel control={<Checkbox checked={selectedRoles.includes("ADMIN")} onChange={() => toggleRole("ADMIN")} />} label="Administrateur" />
              </FormGroup>
      ...
      <RoleChangedDialog
        ...
        onConfirm={() => {
          logout();
          navigate("/login", { state: { message: "Vos rôles ont été modifiés. Reconnectez-vous." } });
        }}
      />
    </>
  );
